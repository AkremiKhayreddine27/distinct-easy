...
import * as fromPayments from "../payments/payments.selector";
import * as fromReservations from "../reservations/reservations.selector";
import { Payment, Reservation, Property } from "../../data/models";
import { Pagination, FilterConf, sort, filter, paginate } from "../helpers";
import { Dictionary } from "@ngrx/entity";
import * as dateFns from "date-fns";

export const getStatsByYear = (
  start: Date = dateFns.startOfYear(new Date()),
  end: Date = new Date()
) =>
  createSelector(fromPayments.getPaymentsState, state => {
    let years = [];
    for (
      let date = start;
      dateFns.isBefore(date, end) || dateFns.isEqual(date, end);
...
export const getRevenuesByPlatforms = platforms =>
  createSelector(fromReservations.getReservationsState, state => {
...
export const getTotalNbrNightsByPlatforms = platforms =>
  createSelector(fromReservations.getReservationsState, state => {
    return [...platforms].map(platform => {
      let p = { ...platform };
      p.nbrNights = 0;
      filter(
        ...
        if (p.id === reservation.platform.id) {
          p.nbrNights =
            p.nbrNights +
            dateFns.differenceInDays(reservation.end, reservation.start);
        }
      });
      return p;
    });
  ...
  if (payment.type.isIncome) {
    switch (payment.status.id) {
      case 1:
        ...
        finances.potentialRevenue =
          finances.potentialRevenue + payment.price.value;
        break;
      case 2:
  ...
  if (payment.type.isOutgo) {
    switch (payment.status.id) {
      case 1:
        ...
        finances.potentialExpenses =
          finances.potentialExpenses + payment.price.value;
        break;
      case 2:
        finances.pendingExpenses =
          finances.pendingExpenses + payment.price.value;
        finances.potentialExpenses =
          finances.potentialExpenses + payment.price.value;
        break;
      case 3:
        finances.inDelayExpenses =
          finances.inDelayExpenses + payment.price.value;
        finances.potentialExpenses =
          finances.potentialExpenses + payment.price.value;
        break;
    }
