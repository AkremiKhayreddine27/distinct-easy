...
export const getPaymentsCount = createSelector(getPaymentsState, state => {
  return filter(
    state.filters,
...
export const getPaymentsByNomenclature = (id: number, type: string) =>
  createSelector(getPaymentsState, state => {
    const filters = {
      filters: [
        {
          field: "nomenclature",
    ...
    return sort(
      sortConf,
      filter(filters, Object.keys(state.entities).map(id => state.entities[id]))
    );
  });

export const getAllPayments = createSelector(getPaymentsState, state => {
