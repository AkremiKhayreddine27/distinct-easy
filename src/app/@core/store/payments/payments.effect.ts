        ...
        .pipe(
            switchMap((action: paymentsActions.UpdateSoujournPayments) => {
                ...
                if (deposit != 0 && action.old.deposit == 0) {
                    depositPayment = this.paymentsService.createDepositPayment(deposit, action.changes, paymentReferences.getStatus(2), false);
                ...
                if (bail != 0 && action.old.bail.value == 0) {
                    bailPayment = this.paymentsService.createBailPayment(bail, action.changes, paymentReferences.getStatus(2), false);
                    const refundPayment = this.paymentsService.createRefundPayment(bail, action.changes, paymentReferences.getStatus(2), false);
