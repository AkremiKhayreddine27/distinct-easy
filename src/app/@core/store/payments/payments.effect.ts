...
import * as paymentsActions from "./payments.action";
import { switchMap, map, catchError } from "rxjs/operators";
import { PaymentsService } from "../../data/services/payments.service";
import * as paymentReferences from "../../data/references/payment.reference";
...
export class PaymentsEffects {
  ...
  @Effect()
  loadPayments$ = this.actions$.ofType(paymentsActions.LOAD_PAYMENTS).pipe(
    switchMap(() => {
  ...
  @Effect()
  createPayment$ = this.actions$.ofType(paymentsActions.CREATE_PAYMENT).pipe(
  ...
  @Effect()
  updatePayment$ = this.actions$.ofType(paymentsActions.UPDATE_PAYMENT).pipe(
    switchMap((action: paymentsActions.UpdatePayment) => {
      return of(this.paymentsService.update(action.id, action.changes)).pipe(
        map(
          () =>
            new paymentsActions.UpdatePaymentSuccess(action.id, action.changes)
        ),
  ...
  @Effect()
  deletedPayment$ = this.actions$.ofType(paymentsActions.DELETE_PAYMENT).pipe(
  ...
  @Effect()
  createSoujournPayments$ = this.actions$
    .ofType(paymentsActions.CREATE_SOUJOURN_PAYMENTS)
    .pipe(
      switchMap((action: paymentsActions.CreateSoujournPayments) => {
        return of(
          this.paymentsService.createSojournPayments(action.parent, true)
        ).pipe(
          map(payments => new paymentsActions.LoadPaymentsSuccess(payments)),
          catchError(error => of(new paymentsActions.LoadPaymentsFail(error)))
        );
  ...
  @Effect()
  updateSoujournPayments$ = this.actions$
    .ofType(paymentsActions.UPDATE_SOUJOURN_PAYMENTS)
    .pipe(
      switchMap((action: paymentsActions.UpdateSoujournPayments) => {
        let actions: paymentsActions.PaymentsAction[] = [];

        const deposit =
          (action.changes.price.value * action.changes.deposit) / 100;
        ...
        if (soldePayment) {
          ...
          actions.push(
            new paymentsActions.UpdatePayment(soldePayment.id, soldePayment)
          );
        }
        ...
        if (deposit != 0 && action.old.deposit == 0) {
          depositPayment = this.paymentsService.createDepositPayment(
            deposit,
            action.changes,
            paymentReferences.getStatus(2),
            false
          );
        ...
        if (bail != 0 && action.old.bail.value == 0) {
          bailPayment = this.paymentsService.createBailPayment(
            bail,
            action.changes,
            paymentReferences.getStatus(2),
            false
          );
          const refundPayment = this.paymentsService.createRefundPayment(
            bail,
            action.changes,
            paymentReferences.getStatus(2),
            false
